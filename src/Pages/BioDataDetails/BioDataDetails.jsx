...
const BioDataDetails = () => {
  const id = useParams();
  const {info2,requested} = useGetBiodataDetails(id);
  const data = useGetSimilarBiodata(info2?.gender);
  const filterBiodata = data.filter(item => item?._id !== info2?._id)
  console.log(data);
  return (
    <Container>
      <div className="flex flex-col lg:flex-row gap-5">
        <div className="flex-1 h-fit">
          <BioDataDetailsCard info={info2} requested={requested} />
        </div>
